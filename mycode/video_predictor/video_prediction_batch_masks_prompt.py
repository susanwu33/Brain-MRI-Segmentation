...
for idx, row in df_sample.iterrows():
    ...
    for i in range(image_data.shape[2]): 
        ...
        # only save the non-empty masks
        if np.any(gt_mask > 0):
            mask_prompts[i] = gt_mask

    print(f"Total Non-empty Masks Found: {len(mask_prompts)}")
    ...
    for i, gt_mask in mask_prompts.items():
        _, out_obj_ids, out_mask_logits = predictor.add_new_mask(inference_state, frame_idx=i, obj_id=1, mask=mask_prompts[i])
