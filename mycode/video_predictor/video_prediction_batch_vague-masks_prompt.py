...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("sam2_inference_vague-masks.log", mode="w"),
...
def show_mask(mask, ax, obj_id=None, color=(1, 0, 1, 0.6)):  # Purple overlay for predictions
    h, w = mask.shape[-2:]
    mask_image = mask.reshape(h, w, 1) * np.array(color).reshape(1, 1, -1)
    ax.imshow(mask_image)

def show_gt_mask(gt_mask, ax, color=(1, 0, 0, 0.4)):  # Red overlay for ground truth
    h, w = gt_mask.shape[-2:]
...
def save_nifti_from_slices(pred_mask_slices, reference_nifti_path, output_nifti_path):
    ...
    logging.info(f"Saved full-size 3D NIfTI mask: {output_nifti_path}")

# ========== Metric Functions (updated) ==========
def calculate_iou(pred_masks, gt_masks):
    """
    Computes per-slice and mean Intersection over Union (IoU).
    If both GT and predicted masks are empty, returns IoU = 1.0.
    """
    iou_scores = []
...
def calculate_dice(pred_masks, gt_masks):
    """
    Computes per-slice and mean Dice coefficient.
    If both GT and predicted masks are empty, returns Dice = 1.0.
    """
    dice_scores = []
    num_slices = gt_masks.shape[2]
    for i in range(num_slices):
...
# ========== Vague Mask Function: Smaller Ellipse ==========
def create_vague_mask_small(gt_mask, scale_factor=0.8, blur_kernel=(15,15)):
    """
    Generates a vague mask by drawing a filled ellipse that is smaller than the GT segmentation.
    It computes the bounding box from the largest contour in the GT mask,
    scales the axes by scale_factor (<1), applies a Gaussian blur, and thresholds the result.
    
    Parameters:
      - gt_mask: Input binary ground truth mask.
      - scale_factor: Factor by which to shrink the ellipse dimensions (e.g., 0.8).
...
# Set workspace directory for results.
workspace_dir = "/home/yw2692/workspace/Brain-MRI-Segmentation/sam2_vis_result_perturbed-mask_prompt"
...
# Lists for overall metric logging.
video_iou_scores = []
video_dice_scores = []
...
for idx, row in df_sample.iterrows():
    ...
    # Construct T2 MRI and GT segmentation paths.
    ...
    perturbed_mask_dir = os.path.join(case_output_dir, "perturbed_masks")
    mask_comparison_dir = os.path.join(case_output_dir, "mask_comparison")
    for dir_path in [case_output_dir, output_vis_dir, gt_output_dir, pred_output_dir, slice_output_dir, perturbed_mask_dir, mask_comparison_dir]:
    ...
    frame_names = []
    mask_prompts = {}      # For slices with non-empty GT, we create vague mask prompts.
    # bounding_boxes = {}    # We compute bounding boxes from GT masks.
    ...
    for i in range(num_slices):
        ...
        # For slices with non-empty GT, create a vague mask prompt.
        if np.any(gt_mask > 0):
            vague_mask = create_vague_mask_small(gt_mask, scale_factor=0.8, blur_kernel=(15,15))
            mask_prompts[i] = vague_mask
            perturbed_mask_filename = os.path.join(perturbed_mask_dir, f"vague_mask_{i:03d}.png")
            cv2.imwrite(perturbed_mask_filename, vague_mask)
            # Create side-by-side comparison of GT vs vague mask.
            gt_mask_vis = (gt_mask * 255).astype(np.uint8)
            comparison = np.hstack((gt_mask_vis, vague_mask))
            comparison_filename = os.path.join(mask_comparison_dir, f"mask_comparison_{i:03d}.png")
            cv2.imwrite(comparison_filename, comparison)

    print(f"Total slices with vague prompt: {len(mask_prompts)}")
    # print(f"Total slices with bounding box: {len(bounding_boxes)}")

    # Initialize SAM2 state.
    inference_state = predictor.init_state(video_path=slice_output_dir)

    # Run segmentation using vague mask prompts.
    video_segments = {}
    for i, vague_mask in mask_prompts.items():
        # Use add_new_mask with the vague mask as prompt.
        _, out_obj_ids, out_mask_logits = predictor.add_new_mask(inference_state, frame_idx=i, obj_id=1, mask=vague_mask)
    ...
    for out_frame_idx, out_obj_ids, out_mask_logits in predictor.propagate_in_video(inference_state, start_frame_idx=0):
        if out_frame_idx not in video_segments:
            video_segments[out_frame_idx] = {
                out_obj_ids[0]: (out_mask_logits[0] > 0.2).cpu().numpy()
            }

    # Save visualizations and collect predicted masks (from video propagation).
    pred_mask_slices = []
    for out_frame_idx in range(num_slices):
        ...
        # Ensure merged mask is 2D.
        merged_pred_mask = np.squeeze(merged_pred_mask).astype(np.uint8)
    ...
    # ===== Metric Set 1: Using Video Propagation Output =====
    ...
    # ===== Metric Set 2: Forced Empty for Slices Without Bounding Box =====
    pred_mask_slices_forced = []
    for i in range(num_slices):
        # If a bounding box was not detected (i not in mask_prompts), force an empty mask.
        if i in mask_prompts:
            pred_mask_slices_forced.append(pred_mask_slices[i])
        else:
            pred_mask_slices_forced.append(np.zeros((gt_masks.shape[0], gt_masks.shape[1]), dtype=np.uint8))
    miou_forced, iou_forced = calculate_iou(pred_mask_slices_forced, gt_masks)
    mdice_forced, dice_forced = calculate_dice(pred_mask_slices_forced, gt_masks)
    logging.info(f"Subject {pid} (Forced Empty for No-Mask): Mean IoU = {miou_forced:.4f}, Mean Dice = {mdice_forced:.4f}")

    # Store metrics for overall logging.
    # iou_scores.append(miou_video)
    ...
    del inference_state, video_segments, mask_prompts, pred_mask_slices, frame_names, image_data, gt_masks #, bounding_boxes
    gc.collect()
