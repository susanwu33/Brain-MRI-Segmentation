...
# Set the environment variable to help reduce GPU fragmentation.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("sam2_inference_points.log", mode="w"),
...
def generate_prompt_points(mask, num_positive=20, num_negative=50):
    """
    Generates point prompts from a binary mask.
    If positive (foreground) points exist, sample a fixed number
    of them (labels=1). If none exist, sample negative points (labels=0).
    Returns:
      - points: A NumPy array of (x, y) coordinates (float32).
      - labels: A NumPy array of labels (1 for foreground, 0 for background).
      - prompt_flag: True if positive points were sampled.
    """
    pos_indices = np.argwhere(mask > 0)
    if len(pos_indices) == 0:
        # No positives: do not add any prompt.
        return None, None, False

    # Otherwise, sample positive points.
    if len(pos_indices) >= num_positive:
        positive_points = pos_indices[np.random.choice(len(pos_indices), num_positive, replace=False)]
    else:
        positive_points = pos_indices
...
for idx, row in df_sample.iterrows():
    ...
    prompt_points = {}
    prompt_labels = {}
    idx_with_prompts = []  # store the indices for which a positive point prompt is generated.
    ...
    for i in range(num_slices):
        ...
        pts, lbls, prompt_flag = generate_prompt_points(gt_mask)
        if prompt_flag:
            # Only add if there are positive points (i.e. GT is non-empty).
            prompt_points[i] = pts
            prompt_labels[i] = lbls
            idx_with_prompts.append(i)

    print(f"Total slices with prompt: {len(idx_with_prompts)}")

    # Initialize SAM2 inference state for this subject.
    inference_state = predictor.init_state(video_path=slice_output_dir)

    # Run segmentation using points as prompt.
    video_segments = {}
    for i in idx_with_prompts:
        pts = prompt_points[i]
        lbls = prompt_labels[i]
        # Use the predictor's method (e.g., add_new_points_or_box) to add prompts.
        _, out_obj_ids, out_mask_logits = predictor.add_new_points_or_box(
            inference_state, frame_idx=i, obj_id=1, points=pts, labels=lbls
        )
    ...
    # Save final 3D segmentation as NIfTI.
    ...
    # ===== Metric Set 2: Forced Empty for Slices Without Prompt =====
    pred_mask_slices_forced = []
    for i in range(num_slices):
        # If no point prompt was generated, force an empty mask.
        if i in idx_with_prompts:
    ...
    logging.info(f"Subject {pid} (Forced Empty for No-Prompt): Mean IoU = {miou_forced:.4f}, Mean Dice = {mdice_forced:.4f}")
    ...
    del inference_state, video_segments, prompt_points, prompt_labels, pred_mask_slices, frame_names, image_data, gt_masks
    gc.collect()
