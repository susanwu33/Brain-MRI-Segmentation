...
def generate_prompt_points(mask, num_positive=10, num_negative=50):
    ...
    if len(pos_indices) == 0:
        ...
        negative_points = neg_indices[np.random.choice(len(neg_indices), num_negative, replace=False)] if len(neg_indices) >= num_negative else neg_indices
        points = negative_points.astype(np.float32)
        labels = np.zeros(len(negative_points), dtype=np.int32)
    else:
        positive_points = pos_indices[np.random.choice(len(pos_indices), num_positive, replace=False)] if len(pos_indices) >= num_positive else pos_indices
        #negative_points = neg_indices[np.random.choice(len(neg_indices), num_negative, replace=False)] if len(neg_indices) >= num_negative else neg_indices
        
        #points = np.vstack((positive_points, negative_points)).astype(np.float32)
        #labels = np.hstack((np.ones(len(positive_points), dtype=np.int32), np.zeros(len(negative_points), dtype=np.int32)))
...
for idx, row in df_sample.iterrows():
    ...
    for frame_idx in points.keys():
        _, out_obj_ids, out_mask_logits = predictor.add_new_points_or_box(inference_state, frame_idx=frame_idx, obj_id=1, points=points[frame_idx], labels=labels[frame_idx])
